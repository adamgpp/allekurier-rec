...
use App\Core\Invoice\Domain\Feature\Exception\InvoiceCreationException;
...
final class InvoiceCreatorService implements InvoiceCreationInterface
{
    ...
    public function createInvoice(Ulid $id, Email $userEmail, Amount $amount): void
    {
        ...
        $this->assertInvoiceCanBeCreated($id, $userEmail, $user);
    ...
    private function assertInvoiceCanBeCreated(Ulid $id, Email $email, ?User $user): void
    {
        if ($this->invoiceReadRepository->existsById($id)) {
            throw InvoiceCreationException::invoiceWithIdAlreadyExists($id);
        }

        if (null === $user) {
            throw InvoiceCreationException::userWithEmailNotExists($email);
        }

        if (false === $user->isActive()) {
            throw InvoiceCreationException::userNotActive($user->getId());
        }
    }
}
